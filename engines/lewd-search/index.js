...
const _slugToTitle = (slug) => slug.replace(/-+/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()).trim();
    ...
    try {
        ...
        $("a[href*='/video/']").each((_, el) => {
            ...
            if (href.startsWith("//"))
                ...
                return;
            if (!href.includes("rule34video.com"))
                return;
            if (seen.has(href))
                return;
            seen.add(href);
            ...
            if (!thumbEl.length)
                thumbEl = $(el).closest("div").find("img").first();
...
export default class LewdSearchEngine {
    ...
    async executeSearch(query, page = 1, _timeFilter, context) {
        ...
        const [eporner, iwara, xvideos, pornhub, xhamster, xnxx, youporn, redtube, tgtube, r34v] = await Promise.allSettled([
        ...
        return _interleave(eporner.status === "fulfilled" ? eporner.value : [], iwara.status === "fulfilled" ? iwara.value : [], xvideos.status === "fulfilled" ? xvideos.value : [], pornhub.status === "fulfilled" ? pornhub.value : [], xhamster.status === "fulfilled" ? xhamster.value : [], xnxx.status === "fulfilled" ? xnxx.value : [], youporn.status === "fulfilled" ? youporn.value : [], redtube.status === "fulfilled" ? redtube.value : [], tgtube.status === "fulfilled" ? tgtube.value : [], r34v.status === "fulfilled" ? r34v.value : []);
    }
}
